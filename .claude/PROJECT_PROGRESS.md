...
## Current Focus
**Phases 1, 2, 3, and 4C shipped.** Frontend resilience + backend tuning + UX polish + cook-tracking are all landed. Next: Phase 4A (link-based recipe sharing) and 4B (my-favorites / not-rated filters + my-rating sort) — independent, pick either.

## Active Tasks
- [NEXT] Phase 4A: Link-based recipe sharing — `share_token` on recipes, public `/r/shared/:token` route, `ShareRecipeDialog`, `shared_recipes_controller` with public show + authenticated copy
